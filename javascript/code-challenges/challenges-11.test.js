...
function transformToLis(obj) {
  ...
  let resultArr = entryArr.map(element => {
    let str = `<li>${element[0]}: ${element[1]}</li>`;
    return str;
  });
  return resultArr;
}
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let inerArr = [];
  let resultArr = input.map(element => {
    // console.log(11111, element);
    inerArr = [];
    element.forEach(element2 => {
      // console.log(55555,typeof (element2));
      if (element2 % 5 === 0 && typeof (element2) === 'number') {
        // console.log(2222, element2);
        inerArr.push(Math.pow(2, element2));
      }
    });
    // console.log(33333,inerArr);
    return inerArr;
  });
  // console.log(input);
