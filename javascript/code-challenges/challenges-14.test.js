...
const screenForNames = (arr) => {
  // Solution code here...
  let regex = /^(Mr||Mrs||Ms||Dr).\s\S[A-Za-z]*/gm;
  let resultArr = [];
  arr.forEach(element => {
    if (element.match(regex) !== null) {
      resultArr.push(element.match(regex)[0]);
    }
  });
  return resultArr;
}
...
const toTitleCase = (arr) => {
  ...
  let resultArr = [];
  arr.forEach((element) => {
    let str = element.charAt(0).toUpperCase() + element.substr(1);
    resultArr.push(str);
  });
  return resultArr;
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < board.length; i++) {
    ...
    for (let j = 0; j < board[i].length; j++) {
      if (i === 0) {
        if (board[i][i] !== '' && board[i][j] === board[i + 1][j] && board[i][j] === board[i + 2][j]) {
          x = true;
          break;
        } else if (board[i][i] !== '' && board[i][j] === board[i + 1][j + 1] && board[i][j] === board[i + 2][j + 2]) {
          x = true;
          break;
        }
      } else if (i === 2) {
        if (board[i][i] !== '' && board[i][j] === board[i - 1][j + 1] && board[j][i] === board[i - 2][j + 2]) {
          x = true;
          break;
        }
      }
